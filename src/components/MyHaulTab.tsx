...
export default function MyHaulTab() {
  return (
    <div style={{ fontFamily: theme.type.family, color: pal.pale, fontSize: 12, lineHeight: 1.6 }}>
      ...
      <h3 style={{ color: pal.amber, marginTop: 12 }}>The collection</h3>
      {ITEMS.map((item) => (
        <div key={item.key} style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '6px 0', borderBottom: `1px solid ${pal.chrome}` }}>
          ...
          <div style={{ flex: 1 }}>
            <div>{item.name} <span style={{ color: pal.concrete, fontSize: 10 }}>({item.rarity})</span></div>
            <div style={{ color: pal.concrete, fontSize: 10 }}>{item.effect}</div>
          </div>
          <span style={{ color: pal.steelLt, fontSize: 10 }}>not earned</span>
        </div>
      ))}
    </div>
  )
